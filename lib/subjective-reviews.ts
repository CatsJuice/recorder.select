...
const zhCN: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "0.5.x 重新设计了原生 macOS 编辑器，采用左侧检查器和可配置的截图预览堆栈。",
    ux: "0.5.2 提供三步首次启动引导：欢迎、权限和首次截图。引导说明各项权限，并提供授权请求或系统设置入口。",
    summary: "免费、开源的截图和录屏应用。0.5.2 包含图像和视频编辑器、光标样式、自定义快捷键、媒体库，以及上传到用户自有存储的分享链接。",
  },
...
const en: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "The 0.5.x redesign includes native macOS editors with left-side inspectors and a configurable capture deck.",
    ux: "0.5.2 provides three-step first-launch onboarding: Welcome, Permissions, and First Capture. It explains each permission and offers permission requests or links to System Settings.",
    summary: "Free, open-source screenshot and recording app. 0.5.2 includes image and video editors, cursor styling, customizable shortcuts, a Media Gallery, and share links backed by user-owned storage.",
  },
...
const zhTW: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "0.5.x 重新設計了原生 macOS 編輯器，採用左側檢查器和可設定的擷取預覽堆疊。",
    ux: "0.5.2 提供三步首次啟動引導：歡迎、權限和首次擷取。引導說明各項權限，並提供授權請求或系統設定入口。",
    summary: "免費、開源的截圖與錄影應用程式。0.5.2 包含影像與影片編輯器、游標樣式、自訂快捷鍵、媒體庫，以及上傳至使用者自有儲存空間的分享連結。",
  },
...
const ja: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "0.5.x の刷新では、左側のインスペクタを備えたネイティブ macOS エディタと、カスタマイズ可能なキャプチャ一覧を導入。",
    ux: "0.5.2 の初回起動ガイドは、ようこそ、権限、最初のキャプチャの3段階。各権限を説明し、許可のリクエストやシステム設定へのリンクを提供する。",
    summary: "無料でオープンソースのスクリーンショット・画面録画アプリ。0.5.2 は画像・動画エディタ、カーソルのスタイル変更、ショートカットのカスタマイズ、メディアギャラリー、自分のストレージを使った共有リンクを備える。",
  },
...
const ko: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "0.5.x에서 새롭게 구성한 네이티브 macOS 편집기는 왼쪽 속성 패널과 사용자 설정이 가능한 캡처 모음을 제공한다.",
    ux: "0.5.2는 첫 실행 시 환영, 권한, 첫 캡처의 3단계 온보딩을 제공한다. 각 권한을 설명하고 권한 요청 또는 시스템 설정 링크를 제공한다.",
    summary: "무료 오픈 소스 스크린샷 및 화면 녹화 앱. 0.5.2에는 이미지·동영상 편집기, 커서 스타일, 사용자 지정 단축키, 미디어 갤러리, 사용자 소유 저장소를 이용한 공유 링크가 포함된다.",
  },
...
const es: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "El rediseño de 0.5.x incluye editores nativos de macOS con inspectores a la izquierda y un panel de capturas configurable.",
    ux: "0.5.2 ofrece una guía de primer inicio en tres pasos: bienvenida, permisos y primera captura. Explica cada permiso y permite solicitarlo o abrir Ajustes del Sistema.",
    summary: "Aplicación gratuita y de código abierto para capturas y grabación de pantalla. 0.5.2 incluye editores de imagen y vídeo, estilos de cursor, atajos personalizables, una galería multimedia y enlaces para compartir desde almacenamiento propio.",
  },
...
const fr: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "La refonte 0.5.x comprend des éditeurs natifs macOS avec des inspecteurs à gauche et un panneau de captures configurable.",
    ux: "0.5.2 propose un parcours de premier lancement en trois étapes : accueil, autorisations et première capture. Chaque autorisation est expliquée, avec une demande d’accès ou un lien vers les Réglages Système.",
    summary: "Application gratuite et open source de capture et d’enregistrement d’écran. 0.5.2 comprend des éditeurs d’images et de vidéos, des styles de curseur, des raccourcis personnalisables, une galerie multimédia et des liens de partage utilisant le stockage de l’utilisateur.",
  },
...
const de: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "Die Überarbeitung in 0.5.x umfasst native macOS-Editoren mit Inspektoren auf der linken Seite und einer konfigurierbaren Aufnahmeübersicht.",
    ux: "0.5.2 führt beim ersten Start durch drei Schritte: Willkommen, Berechtigungen und erste Aufnahme. Jede Berechtigung wird erklärt; Zugriffsanfragen oder Links zu den Systemeinstellungen sind direkt verfügbar.",
    summary: "Kostenlose Open-Source-App für Screenshots und Bildschirmaufnahmen. 0.5.2 bietet Bild- und Videoeditoren, Cursorstile, anpassbare Tastenkürzel, eine Mediengalerie und Freigabelinks über eigenen Speicher.",
  },
...
const ptBR: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "A reformulação da versão 0.5.x inclui editores nativos do macOS com painéis de propriedades à esquerda e um painel de capturas configurável.",
    ux: "A versão 0.5.2 oferece uma orientação inicial em três etapas: boas-vindas, permissões e primeira captura. Explica cada permissão e permite solicitar acesso ou abrir os Ajustes do Sistema.",
    summary: "Aplicativo gratuito e de código aberto para capturas e gravação de tela. A versão 0.5.2 inclui editores de imagem e vídeo, estilos de cursor, atalhos personalizáveis, galeria de mídia e links de compartilhamento usando armazenamento do próprio usuário.",
  },
