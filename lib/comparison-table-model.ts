...
export function createComparisonModel(options: ModelOptions) {
  ...
  if (options.subjectiveReviewsExpanded) subjectiveReviewKeys.forEach(key => {
    ...
    add({ id: `review-${key}`, label: t(key === 'ui' ? 'subjectiveUi' : key === 'ux' ? 'subjectiveUx' : 'subjectiveSummary'), level: 1, height: Math.max(72, lines * 20 + 32), review: true, cell: column => ({ text: texts[column], muted: texts[column] === '—', caution: key === 'summary' && ['glisio', 'screeen', 'screenmovie', 'recordly'].includes(products[column].id) }) });
  });
