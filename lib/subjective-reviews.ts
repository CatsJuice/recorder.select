...
const zhCN: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "目前仍在 ScreenDrop 基础上大量复用基础组件修改了一些排版，导致 UI 有粗有细，有割裂感",
    ux: "仍存在一些引导体验问题，例如按键录制需要到设置里面手动开启",
    summary: "仓库对比得出，当前 BetterShot 的 Swift 源码，约 71% 的非空行与 Screendrop 匹配；去掉整行注释和纯标点行后约 68%。最初引入的 0.4.0 版本约为 84%。没有来源标注",
  },
...
const en: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "The current UI still extensively reuses foundational ScreenDrop components with some layout changes, resulting in uneven polish and a disjointed feel.",
    ux: "Some onboarding issues remain; for example, keystroke recording must be enabled manually in Settings.",
    summary: "Repository comparison shows that approximately 71% of nonblank lines in BetterShot’s current Swift source match Screendrop; excluding full-line comments and punctuation-only lines, the figure is approximately 68%. The initially introduced 0.4.0 version was approximately 84%. No source attribution is provided.",
  },
...
const zhTW: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "目前仍在 ScreenDrop 基礎上大量重用基礎元件並修改部分排版，導致 UI 精緻度不一，有割裂感。",
    ux: "仍存在一些引導體驗問題，例如按鍵錄製需要到設定中手動開啟。",
    summary: "倉庫比對顯示，目前 BetterShot 的 Swift 原始碼約有 71% 的非空白行與 Screendrop 相符；排除整行註解及純標點行後約為 68%。最初引入的 0.4.0 版本約為 84%。未標註來源。",
  },
...
const ja: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "現在も ScreenDrop の基本コンポーネントを大幅に再利用し、一部のレイアウトを変更しているため、UI の仕上がりにばらつきがあり、統一感に欠ける。",
    ux: "導入時の案内にはまだ問題がある。たとえば、キー入力の記録は設定で手動で有効にする必要がある。",
    summary: "リポジトリの比較では、現在の BetterShot の Swift ソースの空行を除いた行の約 71% が Screendrop と一致する。行全体のコメントと記号のみの行を除くと約 68%。最初に導入された 0.4.0 では約 84% だった。出典の明記はない。",
  },
...
const ko: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "현재도 ScreenDrop의 기본 컴포넌트를 대폭 재사용하면서 일부 배치를 바꾸어, UI의 완성도가 고르지 않고 통일감이 부족하다.",
    ux: "온보딩에는 여전히 문제가 있다. 예를 들어 키 입력 녹화는 설정에서 수동으로 켜야 한다.",
    summary: "저장소 비교 결과, 현재 BetterShot의 Swift 소스에서 빈 줄을 제외한 줄의 약 71%가 Screendrop과 일치한다. 줄 전체 주석과 문장 부호만 있는 줄을 제외하면 약 68%다. 처음 도입된 0.4.0 버전은 약 84%였다. 출처 표시는 없다.",
  },
...
const es: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "La interfaz actual sigue reutilizando ampliamente componentes básicos de ScreenDrop con algunos cambios de distribución, lo que produce un acabado desigual y una sensación de falta de cohesión.",
    ux: "Persisten algunos problemas en la guía inicial; por ejemplo, la grabación de pulsaciones debe activarse manualmente en Ajustes.",
    summary: "La comparación de repositorios muestra que aproximadamente el 71% de las líneas no vacías del código Swift actual de BetterShot coincide con Screendrop; al excluir los comentarios de línea completa y las líneas que solo contienen signos de puntuación, la cifra es de aproximadamente el 68%. En la versión 0.4.0 introducida inicialmente era de aproximadamente el 84%. No se indica la fuente.",
  },
...
const fr: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "L’interface actuelle réutilise encore largement les composants de base de ScreenDrop avec quelques changements de disposition, ce qui donne une finition inégale et une impression de manque de cohérence.",
    ux: "Certains problèmes d’accompagnement initial persistent ; par exemple, l’enregistrement des touches doit être activé manuellement dans les réglages.",
    summary: "La comparaison des dépôts montre qu’environ 71% des lignes non vides du code Swift actuel de BetterShot correspondent à Screendrop ; après exclusion des commentaires occupant une ligne entière et des lignes composées uniquement de signes de ponctuation, ce chiffre est d’environ 68%. Pour la version 0.4.0 introduite initialement, il était d’environ 84%. Aucune attribution de source n’est fournie.",
  },
...
const de: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "Die aktuelle Oberfläche verwendet weiterhin in großem Umfang grundlegende ScreenDrop-Komponenten mit einigen Layoutänderungen. Dadurch wirkt die UI unterschiedlich ausgereift und uneinheitlich.",
    ux: "Bei der Einführung bestehen weiterhin Probleme; beispielsweise muss die Aufzeichnung von Tastendrücken manuell in den Einstellungen aktiviert werden.",
    summary: "Der Repository-Vergleich zeigt, dass ungefähr 71% der nicht leeren Zeilen im aktuellen Swift-Quellcode von BetterShot mit Screendrop übereinstimmen. Ohne ganzzeilige Kommentare und Zeilen, die nur Satzzeichen enthalten, sind es ungefähr 68%. Bei der ursprünglich eingeführten Version 0.4.0 waren es ungefähr 84%. Eine Quellenangabe fehlt.",
  },
...
const ptBR: Record<string, SubjectiveReview> = {
  ...
  bettershot: {
    ui: "A interface atual ainda reutiliza amplamente componentes básicos do ScreenDrop com algumas mudanças de disposição, resultando em um acabamento desigual e uma sensação de falta de coesão.",
    ux: "Ainda há problemas na orientação inicial; por exemplo, a gravação de teclas precisa ser ativada manualmente nos Ajustes.",
    summary: "A comparação dos repositórios mostra que aproximadamente 71% das linhas não vazias do código Swift atual do BetterShot coincidem com o Screendrop; excluindo comentários de linha inteira e linhas compostas apenas por sinais de pontuação, o valor é de aproximadamente 68%. Na versão 0.4.0 introduzida inicialmente, era de aproximadamente 84%. Não há atribuição da fonte.",
  },
